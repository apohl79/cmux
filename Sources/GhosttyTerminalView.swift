...
private enum TerminalExternalURLOpener {
...
class GhosttyApp {
    ...
    fileprivate static func openEmbeddedBrowserLink(
        ...
        guard BrowserAvailabilitySettings.isEnabled() else {
            ...
            #endif
            return TerminalExternalURLOpener.open(url)
        }
    ...
    private func handleAction(target: ghostty_target_s, action: ghostty_action_s) -> Bool {
        ...
        case GHOSTTY_ACTION_OPEN_URL:
            ...
            if !BrowserLinkOpenSettings.openTerminalLinksInCmuxBrowser() {
                ...
                #endif
                return performOnMain {
                    TerminalExternalURLOpener.open(target.url)
                }
            }
            switch target {
            case let .external(url):
                ...
                #endif
                return performOnMain {
                    TerminalExternalURLOpener.open(url)
                }
...
class GhosttyNSView: NSView, NSUserInterfaceValidations {
    ...
    override func menu(for event: NSEvent) -> NSMenu? {
        ...
        let menu = NSMenu()
        if let url = probedURL, TerminalExternalURLOpener.canOpen(url) {
    ...
    //
    // Cell-column to string-character mapping is approximate for non-ASCII
